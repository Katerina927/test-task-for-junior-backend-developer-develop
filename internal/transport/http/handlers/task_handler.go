...
func (h *TaskHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	created, err := h.usecase.Create(r.Context(), taskusecase.CreateInput{
		...
		Status:      req.Status,
		Periodicity: periodicityDTOToDomain(req.Periodicity),
	})
	if err != nil {
...
func (h *TaskHandler) GenerateForDate(w http.ResponseWriter, r *http.Request) {
	...
	if dateStr == "" {
		writeError(w, http.StatusBadRequest, errors.New("query parameter 'date' is required"))
		return
	}
	...
	tasks, err := h.usecase.GenerateForDate(r.Context(), date)
	if err != nil {
		writeUsecaseError(w, err)
		return
	}
